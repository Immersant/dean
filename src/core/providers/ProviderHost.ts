...
import type { DeanSettings } from '../types';
...
export interface ProviderHost {
  ...
  readonly settings: DeanSettings;
  readonly storage: SharedAppStorage;
  readonly manifest?: { version?: string };

  saveSettings(): Promise<void>;
  mutateSettings(
    mutation: (settings: DeanSettings) => void | Promise<void>,
  ): Promise<void>;
  mutateSettingsConditionally(
    mutation: (settings: DeanSettings) => boolean | Promise<boolean>,
  ): Promise<void>;
