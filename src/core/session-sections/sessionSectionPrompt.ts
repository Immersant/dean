...
- Bound \`kind: collect\` - in-note questionnaire attached to the current conversation. The user fills answers by editing the note. Optional co-located \`actions\` are Act buttons (for example "I'm done"), not a chat Submit modal. When the user clicks Act, Dean sends the merged questions and current answers on that turn. Write the Act \`prompt\` as if those answers are already in context; do not tell the model to Read the fence to recover them.
- Any bound action may set optional \`startNewChat: true\` to open its prompt, plus any resolved Collect answers, as a fresh unsent chat draft instead of sending it to the bound conversation.
