...
class ForkTargetModal extends Modal {
  ...
  onOpen() {
    ...
    this.modalEl.addClass('dean-fork-target-modal');

    const list = this.contentEl.createDiv({ cls: 'dean-fork-target-list' });
  ...
  private createOption(container: HTMLElement, target: ForkTarget, label: string): void {
    const item = container.createDiv({ cls: 'dean-fork-target-option', text: label });
