...
export class InstructionModal extends Modal {
  ...
  onOpen() {
    ...
    contentEl.addClass('dean-instruction-modal');
    this.setTitle('Add custom instruction');

    // User input section (always visible)
    const inputSection = contentEl.createDiv({ cls: 'dean-instruction-section' });
    const inputLabel = inputSection.createDiv({ cls: 'dean-instruction-label' });
    inputLabel.setText('Your input:');
    const inputText = inputSection.createDiv({ cls: 'dean-instruction-original' });
    inputText.setText(this.rawInstruction);

    // Main content section (changes based on state)
    this.contentSectionEl = contentEl.createDiv({ cls: 'dean-instruction-content-section' });

    // Loading state
    this.loadingEl = this.contentSectionEl.createDiv({ cls: 'dean-instruction-loading' });
    this.loadingEl.createDiv({ cls: 'dean-instruction-spinner' });
    this.loadingEl.createSpan({ text: 'Processing your instruction...' });

    // Clarification state (hidden initially)
    this.clarificationEl = this.contentSectionEl.createDiv({ cls: 'dean-instruction-clarification-section' });
    this.clarificationEl.addClass('dean-hidden');
    this.clarificationTextEl = this.clarificationEl.createDiv({ cls: 'dean-instruction-clarification' });

    const responseSection = this.clarificationEl.createDiv({ cls: 'dean-instruction-section' });
    const responseLabel = responseSection.createDiv({ cls: 'dean-instruction-label' });
    responseLabel.setText('Your response:');

    this.responseTextarea = new TextAreaComponent(responseSection);
    this.responseTextarea.inputEl.addClass('dean-instruction-response-textarea');
    ...
    this.confirmationEl = this.contentSectionEl.createDiv({ cls: 'dean-instruction-confirmation-section' });
    this.confirmationEl.addClass('dean-hidden');

    // Refined instruction display/edit
    const refinedSection = this.confirmationEl.createDiv({ cls: 'dean-instruction-section' });
    const refinedLabel = refinedSection.createDiv({ cls: 'dean-instruction-label' });
    refinedLabel.setText('Refined snippet:');

    this.refinedDisplayEl = refinedSection.createDiv({ cls: 'dean-instruction-refined' });
    this.editContainerEl = refinedSection.createDiv({ cls: 'dean-instruction-edit-container' });
    this.editContainerEl.addClass('dean-hidden');

    this.editTextarea = new TextAreaComponent(this.editContainerEl);
    this.editTextarea.inputEl.addClass('dean-instruction-edit-textarea');
    this.editTextarea.inputEl.rows = 4;

    // Buttons (changes based on state)
    this.buttonsEl = contentEl.createDiv({ cls: 'dean-instruction-buttons' });
    this.updateButtons();
  ...
  showClarificationLoading() {
    ...
    if (this.loadingEl) {
      this.loadingEl.querySelector('.dean-instruction-spinner');
  ...
  private showState(state: ModalState) {
    ...
    if (this.loadingEl) {
      this.loadingEl.toggleClass('dean-hidden', state !== 'loading');
    }
    if (this.clarificationEl) {
      this.clarificationEl.toggleClass('dean-hidden', state !== 'clarification');
    }
    if (this.confirmationEl) {
      this.confirmationEl.toggleClass('dean-hidden', state !== 'confirmation');
    }
  ...
  private updateButtons() {
    ...
    const cancelBtn = this.buttonsEl.createEl('button', {
      text: 'Cancel',
      cls: 'dean-instruction-btn dean-instruction-reject-btn',
    ...
    if (this.state === 'clarification') {
      const submitBtn = this.buttonsEl.createEl('button', {
        text: 'Submit',
        cls: 'dean-instruction-btn dean-instruction-accept-btn',
      ...
      this.editBtnEl = this.buttonsEl.createEl('button', {
        text: 'Edit',
        cls: 'dean-instruction-btn dean-instruction-edit-btn',
  ...
  private toggleEdit() {
    ...
    if (this.isEditing) {
      this.refinedDisplayEl?.addClass('dean-hidden');
      this.editContainerEl?.removeClass('dean-hidden');
      ...
      if (this.refinedDisplayEl) {
        ...
        this.refinedDisplayEl.removeClass('dean-hidden');
      }
      this.editContainerEl?.addClass('dean-hidden');
