...
export class EnvSnippetModal extends Modal {
  ...
  onOpen() {
    ...
    this.modalEl.addClass('dean-env-snippet-modal');
    ...
    const renderContextLimitFields = () => {
      ...
      if (uniqueModelIds.size === 0) {
        contextLimitsContainer.addClass('dean-hidden');
        return;
      }

      contextLimitsContainer.removeClass('dean-hidden');
      ...
      for (const modelId of uniqueModelIds) {
        const row = contextLimitsContainer.createDiv({ cls: 'dean-snippet-limit-row' });
        row.createSpan({ text: modelId, cls: 'dean-snippet-limit-model' });
        row.createSpan({ cls: 'dean-snippet-limit-spacer' });

        const aliasInput = row.createEl('input', {
          type: 'text',
          placeholder: t('settings.customModelAliases.placeholder'),
          cls: 'dean-snippet-alias-input',
        });
    ...
    envVarsSetting.settingEl.addClass('dean-env-snippet-setting');
    envVarsSetting.controlEl.addClass('dean-env-snippet-control');

    contextLimitsContainer = contentEl.createDiv({ cls: 'dean-snippet-context-limits' });
    renderContextLimitFields();

    const buttonContainer = contentEl.createDiv({ cls: 'dean-snippet-buttons' });
...
export class EnvSnippetManager {
  ...
  private render() {
    ...
    const headerEl = this.containerEl.createDiv({ cls: 'dean-snippet-header' });
    headerEl.createSpan({ text: t('settings.envSnippets.name'), cls: 'dean-snippet-label' });
    ...
    if (snippets.length === 0) {
      const emptyEl = this.containerEl.createDiv({ cls: 'dean-snippet-empty' });
      emptyEl.setText(t('settings.envSnippets.noSnippets'));
      return;
    }

    const listEl = this.containerEl.createDiv({ cls: 'dean-snippet-list' });

    for (const snippet of snippets) {
      const itemEl = listEl.createDiv({ cls: 'dean-snippet-item' });

      const infoEl = itemEl.createDiv({ cls: 'dean-snippet-info' });

      const nameEl = infoEl.createDiv({ cls: 'dean-snippet-name' });
      nameEl.setText(snippet.name);

      if (snippet.description) {
        const descEl = infoEl.createDiv({ cls: 'dean-snippet-description' });
        descEl.setText(snippet.description);
      }

      const actionsEl = itemEl.createDiv({ cls: 'dean-snippet-actions' });
  ...
  private async insertSnippet(snippet: EnvSnippet) {
    ...
    const view = this.plugin.app.workspace.getLeavesOfType('dean-view')[0]?.view as {
  ...
  private syncTextareaValue(scope: EnvironmentScope, value: string): void {
    const selector = `.dean-settings-env-textarea[data-env-scope="${scope}"]`;
