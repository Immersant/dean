...
export class SlashCommandModal extends Modal {
  ...
  onOpen() {
    ...
    this.modalEl.addClass('dean-sp-modal');
    ...
    const updateSkillOnlyFields = () => {
      ...
      disableUserSetting.settingEl.toggleClass('dean-hidden', !isSkillType);
      if (!isSkillType) {
    ...
    const details = contentEl.createEl('details', { cls: 'dean-sp-advanced-section' });
    details.createEl('summary', {
      text: 'Advanced options',
      cls: 'dean-sp-advanced-summary',
    });
    if (
      ...
      .addToggle(toggle => {
        toggle.setValue(contextValue === 'fork')
          .onChange(value => {
            ...
            agentSetting.settingEl.toggleClass('dean-hidden', !value);
          });
      });
    ...
    agentSetting.settingEl.toggleClass('dean-hidden', contextValue !== 'fork');
    ...
    const buttonContainer = contentEl.createDiv({ cls: 'dean-sp-modal-buttons' });
...
export class SlashCommandSettings {
  ...
  private renderUnavailable(): void {
    ...
    const emptyEl = this.containerEl.createDiv({ cls: 'dean-sp-empty-state' });
  ...
  private render(): void {
    ...
    const headerEl = this.containerEl.createDiv({ cls: 'dean-sp-header' });
    headerEl.createSpan({ text: t('settings.slashCommands.name'), cls: 'dean-sp-label' });

    const actionsEl = headerEl.createDiv({ cls: 'dean-sp-header-actions' });
    ...
    const listEl = this.containerEl.createDiv({ cls: 'dean-sp-list' });
  ...
  private renderCommandItem(listEl: HTMLElement, cmd: ProviderCommandEntry): void {
    const itemEl = listEl.createDiv({ cls: 'dean-sp-item' });

    const infoEl = itemEl.createDiv({ cls: 'dean-sp-info' });

    const headerRow = infoEl.createDiv({ cls: 'dean-sp-item-header' });

    const nameEl = headerRow.createSpan({ cls: 'dean-sp-item-name' });
    nameEl.setText(`/${cmd.name}`);

    if (isSkillEntry(cmd)) {
      headerRow.createSpan({ text: 'skill', cls: 'dean-slash-item-badge' });
    }

    if (cmd.argumentHint) {
      const hintEl = headerRow.createSpan({ cls: 'dean-slash-item-hint' });
      hintEl.setText(cmd.argumentHint);
    }

    if (cmd.description) {
      const descEl = infoEl.createDiv({ cls: 'dean-sp-item-desc' });
      descEl.setText(cmd.description);
    }

    const actionsEl = itemEl.createDiv({ cls: 'dean-sp-item-actions' });
