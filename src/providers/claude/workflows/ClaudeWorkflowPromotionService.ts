...
export class ClaudeWorkflowPromotionService implements ProviderWorkflowPromotionService {
  ...
  async decide(request: WorkflowPromotionRequest): Promise<WorkflowPromotionDecision> {
    ...
    try {
      ...
      return parseClaudeWorkflowPromotionResponse(response, request.targetCandidates);
    } catch {
...
export function parseClaudeWorkflowPromotionResponse(
  response: string,
