...
function createImageHtml(
  ...
  return `<span class="dean-embedded-image"><img src="${escapeHtml(src)}" alt="${alt}" loading="lazy"${style}></span>`;
}

function createFallbackHtml(wikilink: string): string {
  return `<span class="dean-embedded-image-fallback">${escapeHtml(wikilink)}</span>`;
}
