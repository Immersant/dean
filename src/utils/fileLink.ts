...
function createWikilink(
  ...
  return parent.createEl('a', {
    cls: 'dean-file-link internal-link',
    text: displayText,
    attr: {
...
function repairEmptyInternalLink(app: App, link: HTMLAnchorElement): void {
  ...
  link.classList.add('dean-file-link');
 ...
 * Handles both our custom .dean-file-link and Obsidian's .internal-link.
 */
export function registerFileLinkHandler(
  ...
  const handler = (event: MouseEvent) => {
    ...
    const link = target.closest('.dean-file-link, .internal-link') as HTMLAnchorElement;
  ...
  const walker = container.ownerDocument.createTreeWalker(
    ...
    NodeFilter.SHOW_TEXT,
    {
      acceptNode(node) {
        ...
        if (parent.closest('pre, code, a, .dean-file-link, .internal-link')) {
          return NodeFilter.FILTER_REJECT;
        }
