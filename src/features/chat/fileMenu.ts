...
export async function addFileToDean(host: FileMenuHost, file: TFile): Promise<boolean> {
  try {
    await host.activateView();
    const appended = host.getView()?.appendToActiveInput(formatVaultFileMention(file.path)) ?? false;
    if (!appended) {
      new Notice('Dean chat is not ready.');
    }
    return appended;
  } catch {
    new Notice('Failed to add file to Dean.');
    return false;
  }
}
  ...
  host.registerEvent(
    host.app.workspace.on('file-menu', (menu, file) => {
      ...
      menu.addItem((item) => item
        .setTitle('Add to Dean')
        .setIcon('message-square-plus')
        .onClick(() => addFileToDean(host, file)));
    }),
  );
}
