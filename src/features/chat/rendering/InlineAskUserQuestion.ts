...
export class InlineAskUserQuestion {
  ...
  render(): void {
    this.rootEl = this.containerEl.createDiv({ cls: 'dean-ask-question-inline' });

    const titleEl = this.rootEl.createDiv({ cls: 'dean-ask-inline-title' });
    ...
    if (!this.config.immediateSelect) {
      this.tabBar = this.rootEl.createDiv({ cls: 'dean-ask-tab-bar' });
      this.renderTabBar();
    }
    this.contentArea = this.rootEl.createDiv({ cls: 'dean-ask-content' });
  ...
  private renderTabBar(): void {
    ...
    for (let idx = 0; idx < this.questions.length; idx++) {
      ...
      const tab = this.tabBar.createSpan({ cls: 'dean-ask-tab' });
      tab.createSpan({ text: this.questions[idx].header, cls: 'dean-ask-tab-label' });
      tab.createSpan({ text: answered ? ' \u2713' : '', cls: 'dean-ask-tab-tick' });
    ...
    const submitTab = this.tabBar.createSpan({ cls: 'dean-ask-tab' });
    submitTab.createSpan({ text: allAnswered ? '\u2713 ' : '', cls: 'dean-ask-tab-submit-check' });
    submitTab.createSpan({ text: 'Submit', cls: 'dean-ask-tab-label' });
  ...
  private renderQuestionTab(idx: number): void {
    ...
    this.contentArea.createDiv({
      text: q.question,
      cls: 'dean-ask-question-text',
    });

    const listEl = this.contentArea.createDiv({ cls: 'dean-ask-list' });
    ...
    for (let optIdx = 0; optIdx < q.options.length; optIdx++) {
      ...
      const row = listEl.createDiv({ cls: 'dean-ask-item' });
      if (isFocused) row.addClass('is-focused');
      if (isSelected) row.addClass('is-selected');

      row.createSpan({ text: isFocused ? '\u203A' : '\u00A0', cls: 'dean-ask-cursor' });
      row.createSpan({ text: `${optIdx + 1}. `, cls: 'dean-ask-item-num' });

      if (isMulti) {
        this.renderMultiSelectCheckbox(row, isSelected);
      }

      const labelBlock = row.createDiv({ cls: 'dean-ask-item-content' });
      const labelRow = labelBlock.createDiv({ cls: 'dean-ask-label-row' });
      labelRow.createSpan({ text: option.label, cls: 'dean-ask-item-label' });

      if (!isMulti && isSelected) {
        labelRow.createSpan({ text: ' \u2713', cls: 'dean-ask-check-mark' });
      }

      if (option.description) {
        labelBlock.createDiv({ text: option.description, cls: 'dean-ask-item-desc' });
      }
    ...
    if (this.canShowCustomInputForQuestion(q)) {
      ...
      const customRow = listEl.createDiv({ cls: 'dean-ask-item dean-ask-custom-item' });
      if (customFocused) customRow.addClass('is-focused');

      customRow.createSpan({ text: customFocused ? '\u203A' : '\u00A0', cls: 'dean-ask-cursor' });
      customRow.createSpan({ text: `${customIdx + 1}. `, cls: 'dean-ask-item-num' });
  ...
  private renderSubmitTab(): void {
    this.contentArea.createDiv({
      ...
      cls: 'dean-ask-review-title',
    });

    const reviewEl = this.contentArea.createDiv({ cls: 'dean-ask-review' });
    ...
    for (let idx = 0; idx < this.questions.length; idx++) {
      ...
      pairEl.createDiv({ text: `${idx + 1}.`, cls: 'dean-ask-review-num' });
      ...
      bodyEl.createDiv({
        ...
        cls: answerText ? 'dean-ask-review-a-text' : 'dean-ask-review-empty',
      });
    ...
    const actionsEl = this.contentArea.createDiv({ cls: 'dean-ask-list' });
    const allAnswered = this.questions.every((_, i) => this.isQuestionAnswered(i));

    const submitRow = actionsEl.createDiv({ cls: 'dean-ask-item' });
    if (this.focusedItemIndex === 0) submitRow.addClass('is-focused');
    if (!allAnswered) submitRow.addClass('is-disabled');
    submitRow.createSpan({ text: this.focusedItemIndex === 0 ? '\u203A' : '\u00A0', cls: 'dean-ask-cursor' });
    submitRow.createSpan({ text: '1. ', cls: 'dean-ask-item-num' });
    submitRow.createSpan({ text: 'Submit answers', cls: 'dean-ask-item-label' });
    ...
    const cancelRow = actionsEl.createDiv({ cls: 'dean-ask-item' });
    if (this.focusedItemIndex === 1) cancelRow.addClass('is-focused');
    cancelRow.createSpan({ text: this.focusedItemIndex === 1 ? '\u203A' : '\u00A0', cls: 'dean-ask-cursor' });
    cancelRow.createSpan({ text: '2. ', cls: 'dean-ask-item-num' });
    cancelRow.createSpan({ text: 'Cancel', cls: 'dean-ask-item-label' });
  ...
  private renderMultiSelectCheckbox(parent: HTMLElement, checked: boolean): void {
    parent.createSpan({
      text: checked ? '[\u2713] ' : '[ ] ',
      cls: `dean-ask-check${checked ? ' is-checked' : ''}`,
    });
  }
  ...
  private updateOptionVisuals(qIdx: number): void {
    ...
    for (let i = 0; i < q.options.length; i++) {
      ...
      if (isMulti) {
        const checkSpan = item.querySelector('.dean-ask-check');
        if (checkSpan) {
          checkSpan.textContent = isSelected ? '[\u2713] ' : '[ ] ';
          checkSpan.toggleClass('is-checked', isSelected);
        }
      } else {
        const labelRow = item.querySelector('.dean-ask-label-row');
        const existingMark = item.querySelector('.dean-ask-check-mark');
  ...
  private updateTabIndicators(): void {
    for (let idx = 0; idx < this.questions.length; idx++) {
      ...
      const tick = tab.querySelector('.dean-ask-tab-tick');
      const answered = this.isQuestionAnswered(idx);
    ...
    if (submitTab) {
      const submitCheck = submitTab.querySelector('.dean-ask-tab-submit-check');
      const allAnswered = this.questions.every((_, i) => this.isQuestionAnswered(i));
    ...
    switch (e.key) {
      ...
      case 'Enter':
        ...
        if (this.focusedItemIndex < q.options.length) {
          ...
          const input = customRow?.querySelector('.dean-ask-custom-text') as HTMLInputElement;
          input?.focus();
        }
        break;
    }
