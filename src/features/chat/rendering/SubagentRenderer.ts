...
function createSection(parentEl: HTMLElement, title: string, bodyClass?: string): SubagentSection {
  const wrapperEl = parentEl.createDiv({ cls: 'dean-subagent-section' });

  const headerEl = wrapperEl.createDiv({ cls: 'dean-subagent-section-header' });
  headerEl.setAttribute('tabindex', '0');
  headerEl.setAttribute('role', 'button');

  const titleEl = headerEl.createDiv({ cls: 'dean-subagent-section-title' });
  titleEl.setText(title);

  const bodyEl = wrapperEl.createDiv({ cls: 'dean-subagent-section-body' });
...
function setPromptText(promptBodyEl: HTMLElement, prompt: string): void {
  promptBodyEl.empty();
  const textEl = promptBodyEl.createDiv({ cls: 'dean-subagent-prompt-text' });
...
function renderSubagentToolContent(contentEl: HTMLElement, toolCall: ToolCallInfo): void {
  ...
  if (!toolCall.result && toolCall.status === 'running') {
    const emptyEl = contentEl.createDiv({ cls: 'dean-subagent-tool-empty' });
...
function setSubagentToolStatus(view: SubagentToolView, status: ToolCallInfo['status']): void {
  view.statusEl.className = 'dean-subagent-tool-status';
...
function updateSubagentToolView(view: SubagentToolView, toolCall: ToolCallInfo): void {
  view.wrapperEl.className = `dean-subagent-tool-item dean-subagent-tool-${toolCall.status}`;
...
function createSubagentToolView(parentEl: HTMLElement, toolCall: ToolCallInfo): SubagentToolView {
  const wrapperEl = parentEl.createDiv({
    cls: `dean-subagent-tool-item dean-subagent-tool-${toolCall.status}`,
  });
  wrapperEl.dataset.toolId = toolCall.id;

  const headerEl = wrapperEl.createDiv({ cls: 'dean-subagent-tool-header' });
  headerEl.setAttribute('tabindex', '0');
  headerEl.setAttribute('role', 'button');

  const iconEl = headerEl.createDiv({ cls: 'dean-subagent-tool-icon' });
  iconEl.setAttribute('aria-hidden', 'true');
  setToolIcon(iconEl, toolCall.name);

  const nameEl = headerEl.createDiv({ cls: 'dean-subagent-tool-name' });
  const summaryEl = headerEl.createDiv({ cls: 'dean-subagent-tool-summary' });
  const statusEl = headerEl.createDiv({ cls: 'dean-subagent-tool-status' });

  const contentEl = wrapperEl.createDiv({ cls: 'dean-subagent-tool-content' });
...
function ensureResultSection(state: SubagentState): SubagentSection {
  ...
  const section = createSection(state.contentEl, 'Result', 'dean-subagent-result-body');
  section.wrapperEl.addClass('dean-subagent-section-result');
...
function setResultText(state: SubagentState, text: string): void {
  ...
  const resultEl = section.bodyEl.createDiv({ cls: 'dean-subagent-result-output' });
...
function hydrateSyncSubagentStateFromStored(state: SubagentState, subagent: SubagentInfo): void {
  ...
  if (subagent.status === 'completed' || subagent.status === 'error') {
    ...
    state.statusEl.className = 'dean-subagent-status status-running';
...
export function createSubagentBlock(
  ...
  const wrapperEl = parentEl.createDiv({ cls: 'dean-subagent-list' });
  wrapperEl.dataset.subagentId = taskToolId;

  const headerEl = wrapperEl.createDiv({ cls: 'dean-subagent-header' });
  headerEl.setAttribute('tabindex', '0');
  headerEl.setAttribute('role', 'button');

  const iconEl = headerEl.createDiv({ cls: 'dean-subagent-icon' });
  iconEl.setAttribute('aria-hidden', 'true');
  setIcon(iconEl, getToolIcon(TOOL_SUBAGENT));

  const labelEl = headerEl.createDiv({ cls: 'dean-subagent-label' });
  labelEl.setText(truncateDescription(description));

  const statusEl = headerEl.createDiv({ cls: 'dean-subagent-status status-running' });
  statusEl.setAttribute('aria-label', 'Status: running');

  const contentEl = wrapperEl.createDiv({ cls: 'dean-subagent-content' });

  const promptSection = createSection(contentEl, 'Prompt', 'dean-subagent-prompt-body');
  promptSection.wrapperEl.addClass('dean-subagent-section-prompt');
  setPromptText(promptSection.bodyEl, prompt);

  const toolsContainerEl = contentEl.createDiv({ cls: 'dean-subagent-tools' });
...
export function finalizeSubagentBlock(
  ...
  state.statusEl.className = 'dean-subagent-status';
...
function renderAsyncContentLikeSync(
  ...
  const resultSection = createSection(contentEl, 'Result', 'dean-subagent-result-body');
  resultSection.wrapperEl.addClass('dean-subagent-section-result');
  const resultEl = resultSection.bodyEl.createDiv({ cls: 'dean-subagent-result-output' });
...
export function createAsyncSubagentBlock(
  ...
  const statusTextEl = headerEl.createDiv({ cls: 'dean-subagent-status-text' });
...
export function markAsyncSubagentOrphaned(state: AsyncSubagentState): void {
  ...
  state.statusEl.className = 'dean-subagent-status status-error';
  state.statusEl.empty();
...
export function renderStoredAsyncSubagent(
  ...
  const statusEl = headerEl.createDiv({ cls: `dean-subagent-status ${statusIconClass}` });
