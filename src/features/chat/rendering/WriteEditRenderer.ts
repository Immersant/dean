...
export function createWriteEditBlock(
  ...
  const wrapperEl = parentEl.createDiv({ cls: 'dean-write-edit-block' });
  wrapperEl.dataset.toolId = toolCall.id;

  // Header (clickable to collapse/expand)
  const headerEl = wrapperEl.createDiv({ cls: 'dean-write-edit-header' });
  headerEl.setAttribute('tabindex', '0');
  headerEl.setAttribute('role', 'button');

  // File icon
  const iconEl = headerEl.createDiv({ cls: 'dean-write-edit-icon' });
  iconEl.setAttribute('aria-hidden', 'true');
  setIcon(iconEl, getToolIcon(toolName));

  const nameEl = headerEl.createDiv({ cls: 'dean-write-edit-name' });
  nameEl.setText(toolName);
  const summaryEl = headerEl.createDiv({ cls: 'dean-write-edit-summary' });
  summaryEl.setText(fileNameOnly(filePath) || 'file');

  // Populated when diff is computed
  const statsEl = headerEl.createDiv({ cls: 'dean-write-edit-stats' });

  const statusEl = headerEl.createDiv({ cls: 'dean-write-edit-status status-running' });
  statusEl.setAttribute('aria-label', 'Status: running');

  // Content area (collapsed by default)
  const contentEl = wrapperEl.createDiv({ cls: 'dean-write-edit-content' });

  // Initial loading state
  const loadingRow = contentEl.createDiv({ cls: 'dean-write-edit-diff-row' });
  const loadingEl = loadingRow.createDiv({ cls: 'dean-write-edit-loading' });
...
export function updateWriteEditWithDiff(state: WriteEditState, diffData: ToolDiffData): void {
  ...
  // Render diff content
  const row = state.contentEl.createDiv({ cls: 'dean-write-edit-diff-row' });
  const diffEl = row.createDiv({ cls: 'dean-write-edit-diff' });
...
export function finalizeWriteEditBlock(state: WriteEditState, isError: boolean): void {
  ...
  state.statusEl.className = 'dean-write-edit-status';
  ...
  if (isError) {
    ...
    if (!state.diffLines) {
      state.contentEl.empty();
      const row = state.contentEl.createDiv({ cls: 'dean-write-edit-diff-row' });
      const errorEl = row.createDiv({ cls: 'dean-write-edit-error' });
    ...
    state.contentEl.empty();
    const row = state.contentEl.createDiv({ cls: 'dean-write-edit-diff-row' });
    const doneEl = row.createDiv({ cls: 'dean-write-edit-done-text' });
    doneEl.setText('DONE');
  }
...
export function renderStoredWriteEdit(
  ...
  const statusEl = headerEl.createDiv({ cls: 'dean-write-edit-status' });
  if (isError) {
  ...
  const row = contentEl.createDiv({ cls: 'dean-write-edit-diff-row' });
