...
export function renderDiffContent(
  ...
  if (allInserts && diffLines.length > NEW_FILE_DISPLAY_CAP) {
    const hunkEl = containerEl.createDiv({ cls: 'dean-diff-hunk' });
    for (const line of diffLines.slice(0, NEW_FILE_DISPLAY_CAP)) {
      const lineEl = hunkEl.createDiv({ cls: 'dean-diff-line dean-diff-insert' });
      const prefixEl = lineEl.createSpan({ cls: 'dean-diff-prefix' });
      prefixEl.setText('+');
      const contentEl = lineEl.createSpan({ cls: 'dean-diff-text' });
      contentEl.setText(line.text || ' ');
    }
    const remaining = diffLines.length - NEW_FILE_DISPLAY_CAP;
    const separator = containerEl.createDiv({ cls: 'dean-diff-separator' });
  ...
  if (hunks.length === 0) {
    // No changes
    const noChanges = containerEl.createDiv({ cls: 'dean-diff-no-changes' });
  ...
  hunks.forEach((hunk, hunkIndex) => {
    ...
    for (const line of hunk.lines) {
      const lineEl = hunkEl.createDiv({ cls: `dean-diff-line dean-diff-${line.type}` });
