...
export class InlinePlanApproval {
  ...
  render(): void {
    ...
    this.rootEl.createDiv({ cls: 'dean-plan-inline-title', text: 'Plan complete' });

    const actionsEl = this.rootEl.createDiv({ cls: 'dean-ask-list' });

    // 1. Implement
    const implementRow = actionsEl.createDiv({ cls: 'dean-ask-item' });
    implementRow.addClass('is-focused');
    implementRow.createSpan({ text: '\u203A', cls: 'dean-ask-cursor' });
    implementRow.createSpan({ text: '1. ', cls: 'dean-ask-item-num' });
    implementRow.createSpan({ text: 'Implement', cls: 'dean-ask-item-label' });
    ...
    const reviseRow = actionsEl.createDiv({ cls: 'dean-ask-item dean-ask-custom-item' });
    reviseRow.createSpan({ text: '\u00A0', cls: 'dean-ask-cursor' });
    reviseRow.createSpan({ text: '2. ', cls: 'dean-ask-item-num' });
    ...
    cancelRow.createSpan({ text: '\u00A0', cls: 'dean-ask-cursor' });
    cancelRow.createSpan({ text: '3. ', cls: 'dean-ask-item-num' });
    ...
    this.rootEl.createDiv({ text: HINTS_TEXT, cls: 'dean-ask-hints' });
  ...
  private updateFocus(): void {
    for (let i = 0; i < this.items.length; i++) {
      ...
      if (i === this.focusedIndex) {
        ...
        if (item.hasClass('dean-ask-custom-item') && this.isInputFocused) {
          const input = item.querySelector('.dean-ask-custom-text') as HTMLInputElement;
          if (input) {
            input.blur();
            this.isInputFocused = false;
