...
export function autoResizeTextarea(textarea: HTMLTextAreaElement): void {
  const viewHeight = textarea.closest('.dean-container')?.clientHeight ?? window.innerHeight;
  const maxHeight = calculateTextareaMaxHeight(viewHeight);

  textarea.setCssProps({
    '--dean-textarea-min-height': `${TEXTAREA_BASE_MIN_HEIGHT}px`,
    '--dean-textarea-max-height': `${maxHeight}px`,
  });
  ...
  textarea.setCssProps({
    '--dean-textarea-min-height': `${minHeight}px`,
    '--dean-textarea-max-height': `${maxHeight}px`,
  });
}
