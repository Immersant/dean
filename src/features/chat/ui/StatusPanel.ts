...
export class StatusPanel {
  ...
  private createPanel(): void {
    ...
    this.panelEl = this.containerEl.createDiv({ cls: 'dean-status-panel' });

    // Bash output container - hidden by default
    this.bashOutputContainerEl = this.panelEl.createDiv({ cls: 'dean-status-panel-bash dean-hidden' });

    this.bashHeaderEl = this.bashOutputContainerEl.createDiv({
      cls: 'dean-tool-header dean-status-panel-bash-header',
    ...
    this.bashContentEl = this.bashOutputContainerEl.createDiv({ cls: 'dean-status-panel-bash-content' });

    // Todo container
    this.todoContainerEl = this.panelEl.createDiv({ cls: 'dean-status-panel-todos dean-hidden' });

    // Todo header (collapsed view)
    this.todoHeaderEl = this.todoContainerEl.createDiv({
      cls: 'dean-status-panel-header',
      attr: { tabindex: '0', role: 'button' },
    });
    ...
    this.todoContentEl = this.todoContainerEl.createDiv({
      cls: 'dean-status-panel-content dean-todo-list-container dean-hidden',
    });
  }
  ...
  private syncPanelVisibility(): void {
    ...
    this.panelEl.toggleClass('dean-status-panel--visible', hasTodos || hasBashOutputs);
  }
  ...
  updateTodos(todos: TodoItem[] | null): void {
    ...
    if (!todos || todos.length === 0) {
      this.todoContainerEl.addClass('dean-hidden');
    ...
    this.todoContainerEl.removeClass('dean-hidden');
    this.syncPanelVisibility();
  ...
  private renderTodoHeader(completedCount: number, totalCount: number, currentTask: TodoItem | undefined): void {
    ...
    // List icon
    const icon = this.todoHeaderEl.createSpan({ cls: 'dean-status-panel-icon' });
    setIcon(icon, getToolIcon(TOOL_TODO_WRITE));

    // Label
    this.todoHeaderEl.createSpan({
      cls: 'dean-status-panel-label',
    ...
    if (!this.isTodoExpanded) {
      ...
      if (completedCount === totalCount && totalCount > 0) {
        const status = this.todoHeaderEl.createSpan({ cls: 'dean-status-panel-status status-completed' });
  ...
  private updateTodoDisplay(): void {
    ...
    // Show/hide content
    this.todoContentEl.toggleClass('dean-hidden', !this.isTodoExpanded);
  ...
  private renderBashOutputs(options: { scroll?: boolean } = {}): void {
    ...
    if (this.currentBashOutputs.size === 0) {
      this.bashOutputContainerEl.addClass('dean-hidden');
      this.syncPanelVisibility();
      return;
    }

    this.bashOutputContainerEl.removeClass('dean-hidden');
    this.syncPanelVisibility();
    ...
    const headerLabelEl = this.bashHeaderEl.createSpan({ cls: 'dean-tool-label' });
    ...
    const previewEl = this.bashHeaderEl.createSpan({ cls: 'dean-tool-current' });
    previewEl.classList.toggle('dean-hidden', !this.isBashExpanded);

    const summaryStatusEl = this.bashHeaderEl.createSpan({ cls: 'dean-tool-status' });
    if (!this.isBashExpanded && latest) {
      ...
      summaryStatusEl.classList.add('dean-hidden');
    }

    this.bashHeaderEl.setAttribute('aria-expanded', String(this.isBashExpanded));

    const actionsEl = this.bashHeaderEl.createSpan({ cls: 'dean-status-panel-bash-actions' });
    ...
    this.bashContentEl.toggleClass('dean-hidden', !this.isBashExpanded);
  ...
  private renderBashEntry(info: PanelBashOutput): HTMLElement {
    const entryEl = createDiv({ cls: 'dean-tool-call dean-status-panel-bash-entry' });

    const entryHeaderEl = entryEl.createDiv({
      cls: 'dean-tool-header',
      attr: { tabindex: '0', role: 'button' },
    });
    ...
    const entryStatusEl = entryHeaderEl.createSpan({ cls: 'dean-tool-status' });
    ...
    const contentEl = entryEl.createDiv({ cls: 'dean-tool-content' });
    const isEntryExpanded = this.bashEntryExpanded.get(info.id) ?? true;
    contentEl.classList.toggle('dean-hidden', !isEntryExpanded);
    ...
    const rowEl = contentEl.createDiv({ cls: 'dean-tool-result-row' });

    const textEl = rowEl.createSpan({ cls: 'dean-tool-result-text' });
  ...
  private appendActionButton(
    ...
    const el = parent.createSpan({
      cls: `dean-status-panel-bash-action dean-status-panel-bash-action-${name}`,
      attr: {
        role: 'button',
        tabindex: '0',
