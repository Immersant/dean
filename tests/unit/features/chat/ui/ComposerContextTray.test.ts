...
describe('ComposerContextTray', () => {
  ...
  it('owns empty-state visibility and renders slots in semantic order', () => {
    ...
    expect(containerEl.querySelectorAll('.dean-context-chip').map((item: any) => item.dataset.contextSlot)).toEqual([
      'current-note',
      'editor-selection',
      'images',
    ]);
  ...
  it('uses separate keyboard-focusable controls for activation and removal', () => {
    ...
    const mainButton = containerEl.querySelector('.dean-context-chip-main');
    const removeButton = containerEl.querySelector('.dean-context-chip-remove');
  ...
  it('collapses content after the first visual row and exposes the hidden count', () => {
    ...
    const chips = containerEl.querySelectorAll('.dean-context-chip');
    ...
    expect(chips[2].hasClass('dean-context-chip--overflow-hidden')).toBe(true);
    expect(chips[3].hasClass('dean-context-chip--overflow-hidden')).toBe(true);
    const moreButton = containerEl.querySelector('.dean-context-more');
    expect(moreButton?.textContent).toBe('+2 more');

    moreButton?.click();

    expect(containerEl.hasClass('dean-context-row--expanded')).toBe(true);
    expect(chips.every((chip: any) => !chip.hasClass('dean-context-chip--overflow-hidden'))).toBe(true);
  ...
  it('does not collapse vertically centered items that share one flex row', () => {
    ...
    expect(containerEl.querySelector('.dean-context-more')?.hasClass('dean-hidden')).toBe(true);
  });
