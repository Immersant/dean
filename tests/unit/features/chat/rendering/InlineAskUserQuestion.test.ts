...
function findRoot(container: any): any {
  return container.querySelector('.dean-ask-question-inline');
}

function findItems(container: any): any[] {
  return container.querySelectorAll('dean-ask-item');
}
...
describe('InlineAskUserQuestion', () => {
  describe('parseQuestions', () => {
    ...
    it('keeps questions that only allow free-form input', () => {
      ...
      expect(resolve).not.toHaveBeenCalled();
      expect(container.querySelector('dean-ask-custom-item')).not.toBeNull();
    });
    ...
    it('deduplicates options with the same label', () => {
      ...
      const items = container.querySelectorAll('dean-ask-item');
      // 2 unique options + 1 custom input row = 3
      const optionLabels = items
        .filter((item: any) => !item.hasClass('dean-ask-custom-item'))
        .map((item: any) => {
          const labelEl = item.querySelector('dean-ask-item-label');
    ...
    it('uses header when provided, falls back to Q index', () => {
      ...
      const { container } = renderWidget(input);
      const tabLabels = container.querySelectorAll('dean-ask-tab-label');
    ...
    it('treats non-boolean multiSelect values as false', () => {
      ...
      const items = findItems(container).filter(
        (i: any) => !i.hasClass('dean-ask-custom-item'),
      );
      items[0]?.click();

      expect(container.querySelector('dean-ask-review-title')?.textContent).toBe('Review your answers');
    });
  ...
  describe('coerceOption / extractLabel', () => {
    it('handles string options', () => {
      ...
      const labels = container
        .querySelectorAll('dean-ask-item-label')
    ...
    it('shows description when provided', () => {
      ...
      const { container } = renderWidget(input);
      const descEl = container.querySelector('dean-ask-item-desc');
  ...
  describe('selectOption', () => {
    it('selects single-select option via click', () => {
      ...
      const submitItems = container.querySelectorAll('dean-ask-item');
    ...
    it('toggles multi-select options', () => {
      ...
      const checks = container.querySelectorAll('dean-ask-check');
      const checkedCount = checks.filter((c: any) => c.hasClass('is-checked')).length;
      expect(checkedCount).toBe(2);

      // Deselect X
      items[0]?.click();
      const checksAfter = container.querySelectorAll('dean-ask-check');
  ...
  describe('question metadata', () => {
    it('shows custom input only when the question allows "other"', () => {
      ...
      expect(noOther.querySelectorAll('dean-ask-custom-item')).toHaveLength(0);
      ...
      expect(withOther.querySelectorAll('dean-ask-custom-item')).toHaveLength(1);
    });
    ...
    it('renders secret free-form questions with password input', () => {
      ...
      const customInput = container.querySelector('dean-ask-custom-text');
  ...
  describe('keyboard navigation', () => {
    ...
    it('Tab navigates to next question tab', () => {
      ...
      const tabs = container.querySelectorAll('dean-ask-tab');
    ...
    it('click on custom row focuses it', () => {
      ...
      const items = findItems(container);
      const customItem = items.find((i: any) => i.hasClass('dean-ask-custom-item'));
      customItem?.click();
    ...
    it('Enter from custom input commits text and advances tab', () => {
      ...
      const customItem = findItems(container).find((i: any) =>
        i.hasClass('dean-ask-custom-item'),
      );
      const inputEl = customItem?.querySelector('.dean-ask-custom-text');
    ...
    it('does not advance from custom input on Enter during IME composition', () => {
      ...
      const customItem = findItems(container).find((item: any) =>
        item.hasClass('dean-ask-custom-item'),
      );
...
describe('InlineAskUserQuestion - immediateSelect mode', () => {
  describe('multi-question fallback', () => {
    it('falls back to tab-bar rendering when questions.length !== 1', () => {
      ...
      const tabBar = container.querySelector('dean-ask-tab-bar');
  ...
  describe('rendering', () => {
    ...
    it('does not render custom input row', () => {
      const input = makeInput([{ question: 'Pick', options: ['A', 'B'] }]);
      const { container } = renderImmediateWidget(input);
      const customItems = container.querySelectorAll('dean-ask-custom-item');
    ...
    it('uses custom title when provided', () => {
      ...
      const title = container.querySelector('dean-ask-inline-title');
    ...
    it('renders headerEl between title and content', () => {
      ...
      headerEl.addClass('dean-ask-approval-info');
      const input = makeInput([{ question: 'Pick', options: ['A'] }]);
      const { container } = renderImmediateWidget(input, { headerEl: headerEl as any });
      const root = findRoot(container);
      expect(root.children.some((c: any) => c.hasClass('dean-ask-approval-info'))).toBe(true);
    });
  });
