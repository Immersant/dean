...
describe('buildOpencodeManagedConfig', () => {
  it('pins OpenCode build, YOLO, safe, and plan prompts to the managed prompt file', () => {
    expect(buildOpencodeManagedConfig({}, '/vault/.dean/opencode/system.md', 'Yishen')).toEqual({
      $schema: 'https://opencode.ai/config.json',
      agent: {
        build: {
          prompt: '{file:/vault/.dean/opencode/system.md}',
        },
        [OPENCODE_YOLO_MODE_ID]: {
          mode: 'primary',
          permission: {
  ...
  it('can create a dedicated aux agent and default it for the process', () => {
    expect(buildOpencodeManagedConfig(
      {},
      '/vault/.dean/opencode/auxiliary/system.md',
      undefined,
      [{
        definition: {
          mode: 'primary',
      ...
      agent: {
        'dean-aux-readonly': {
          ...
          prompt: '{file:/vault/.dean/opencode/auxiliary/system.md}',
        },
      },
      default_agent: 'dean-aux-readonly',
    });
  });
  ...
  it('merges the user config instead of replacing it', () => {
    expect(buildOpencodeManagedConfig({
      ...
      username: 'Existing',
    }, '/vault/.dean/opencode/system.md')).toEqual({
      $schema: 'https://opencode.ai/config.json',
      agent: {
        build: {
          model: 'openai/gpt-5',
...
describe('prepareOpencodeLaunchArtifacts', () => {
  it('layers the managed prompt config on top of OPENCODE_CONFIG', async () => {
    const tmpRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'dean-opencode-artifacts-'));
