...
describe('systemPrompt', () => {
  describe('buildSystemPrompt', () => {
    it('should produce identical output for the default and explicit Dean profiles', () => {
      const settings = {
      ...
      expect(buildSystemPrompt(settings)).toBe(
        buildSystemPrompt(settings, { toolGuidanceProfile: 'dean' }),
      );
    });
    ...
    it('should omit Dean tool recipes from the provider-native profile', () => {
      const prompt = buildSystemPrompt(
        { mediaFolder: 'attachments' },
        { toolGuidanceProfile: 'provider-native' },
      );
  ...
  describe('computeSystemPromptKey', () => {
    it('includes the tool guidance profile in the prompt key', () => {
      ...
      const deanKey = computeSystemPromptKey(settings, {
        toolGuidanceProfile: 'dean',
      });
      const providerNativeKey = computeSystemPromptKey(settings, {
        toolGuidanceProfile: 'provider-native',
      });

      expect(defaultKey).toBe(deanKey);
      expect(providerNativeKey).not.toBe(deanKey);
    });
