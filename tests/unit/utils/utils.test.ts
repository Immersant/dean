...
describe('utils.ts', () => {
  ...
  describe('expandHomePath', () => {
    const envKey = 'DEAN_TEST_PATH';
    const envValue = path.join(os.tmpdir(), 'dean-env');
    ...
    it('should leave unknown environment variables untouched', () => {
      expect(expandHomePath('%DEAN_MISSING_VAR%')).toBe('%DEAN_MISSING_VAR%');
      expect(expandHomePath('$DEAN_MISSING_VAR')).toBe('$DEAN_MISSING_VAR');
    });
  });
  ...
  describe('normalizePathForFilesystem', () => {
    ...
    it('expands environment variables before filesystem use', () => {
      const envKey = 'DEAN_FS_TEST_PATH';
